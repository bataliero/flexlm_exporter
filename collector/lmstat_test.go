...
func TestParseLmstatLicenseInfoFeature(t *testing.T) {
	...
	for username, licused := range licUsersByFeature["feature102"] {
		var totalLic float64
		for _, lu := range licused {
			totalLic += lu.num
			if lu.version != "v6.3" {
				t.Fatalf("Unexpected version for feature102[%s]: %v!=v6.3", username, lu.version)
			}

			if lu.hostname != "hostname" {
				t.Fatalf("Unexpected hostname for feature102[%s]: %v!=hostname", username, lu.hostname)
			}
		}
		...
		if username == "user" {
			foundUser102 = true
			if totalLic != 1.0 {
				t.Fatalf("Unexpected values for feature102[%s]: %v!=1", username, totalLic)
			}
		}
	}

	if !foundUser102 {
		t.Fatalf("Couldn't parse user \"<user>\" from feature102")
	}
...
func TestParseLmstatLicenseInfoUserSince(t *testing.T) {
	...
	// Test new format with PID information
	...
	// the year does not matter in this case, since lmstat omits the year information
	ctime = time.Now()
	sinceUser2 := time.Date(ctime.Year(), 2, 10, 0, 10, 0, 0, time.UTC)
	sinceUser3 := time.Date(ctime.Year(), 2, 10, 0, 10, 0, 0, time.UTC)
	...
	// for comparison, the unix time is converted to time.Time in time.Local zone. time.Local is used while parsing
	// the lmstat output, so it must be used in the test as well
	for username, licused := range licUsersByFeature["MATLAB"] {
		for i := range licused {
			switch username {
			case "user2":
				utime, err := strconv.ParseInt(licused[i].since, 10, 64)
				...
				since := time.Unix(utime, 0).In(time.UTC)
				if !compareTime(since, sinceUser2) {
					t.Fatalf("Unexpected values for start time (day, month, hour, minute) [%s]: %s !~= %s",
						username, since.String(), sinceUser1.String())
				}
			case "user3":
				...
				since := time.Unix(utime, 0).In(time.UTC)
				if !compareTime(since, sinceUser3) {
					t.Fatalf("Unexpected values for start time (day, month, hour, minute) [%s]: %s !~= %s",
						username, since.String(), sinceUser1.String())
				}
			}
